...
two project facts together: the Icon Composer source exists in the assets folder, and both
...
struct IconComposerTests {
	...
	func usesNifroIconComposerSource() throws {
		let source = Self.repository.appending(path: "assets/Nifro.icon/icon.json")
		let project = Self.repository.appending(path: "Tools/Nifro.xcodeproj/project.pbxproj")
