...
Noren exists because the app it forked from had five years of users and no
...
- Open a [Site submission issue](https://github.com/PathGao/noren/issues/new?template=site_submission.yml)
...
Use the [bug report form](https://github.com/PathGao/noren/issues/new?template=bug_report.yml).
...
```
git clone https://github.com/PathGao/noren.git
open noren/Plash.xcodeproj
```
...
Noren is an independent fork of [Plash](https://github.com/sindresorhus/Plash)
...
- **Noren is not Plash and must never claim to be.** Contributions that
  reintroduce the Plash name, icon, artwork, App Store identity or website as
  Noren's own will be rejected. This is not negotiable, and it applies to
...
- **Don't file our bugs upstream, and don't file upstream's bugs here.**
  Report bugs you see in Noren here. Post-2.16.0 Plash is a different program
  and we can't fix it.
...
Contributions are made under the repository's MIT licence, the same one Noren
