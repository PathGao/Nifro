...
>
> **照着代码重新确认仍然开着的，不是凭信任顺延下来的：** K20、K30、K38、K39、W1、W3–W6、
> W8、W9、E25 和 R6。**K37 是从另一头关掉的：**它声称一台卖掉的显示器根本没有任何办法忘掉，
...
```
未完成    W1 W3-W6 W8 W9 接线   K1 K6 K8 K20 K30 K38 K39 缺陷   L1-L4  V1 V2 V4 V5  S1 S2 S4  D4 D6  E21 E23 E25  U2 U3
...
**剩下的不是一个形状。** 上表里那七条还开着的条目彼此毫无共同点：一个被改写的地址、三十条帮助文案、一个
图标、一个轮询频率、一份不清扫的缓存、每次重载一个进程，以及一个死掉的 `??`。这比前两轮的待办更健康，
也意味着下一个*新*形状的缺陷没有一个簇可以藏在后面。
