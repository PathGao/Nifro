...
> 2026-08-28 对着 `4032ff2` 重新核过——它是 v0.1.3 加上 #56 到 #65。已经做完的条目是删掉而不是划掉——
...
>
> **照着代码重新确认仍然开着的，不是凭信任顺延下来的：** K12、K20、K21、K23、K26、K30、K33、K38、K39、
> W1–W6、W8、W9、E25 和 R6。**K37 是从另一头关掉的：**它声称一台卖掉的显示器根本没有任何办法忘掉，
...
```
未完成    W1-W6 W8 W9 接线   K1 K6 K8 K12 K20 K21 K23 K26 K30 K33 K38 K39 缺陷   L1-L4  V1 V2 V4 V5  S1 S2 S4  D4 D6  E21-E23 E25  U2 U3
...
`git show 54cac6a~1:Nifro/App/Menus.swift`。下面这八条都在 `4032ff2` 上重读过，八条全都仍然成立。
...
| **K23** | 在已关掉的显示器上选播放列表，屏幕仍然是黑的 | **2026-08-28 重新定位，它取代的那条是假的。** 原条目说 `show()` 是「一个光秃秃的 `makeCurrent`，没有那道守卫」——但 `makeCurrent(_:on:switchingDisplayOn:)` 这个参数默认就是 `true`，它自己会唤醒显示器，所以只要这个参数存在，选**网站**就一直会点亮屏幕。那条是在这个参数出现之前写的，之后从没对着被调用方重读过——正是这份文档自己点名的那种失败方式。真正成立的在隔壁一层：`selectPlaylist` 直接写 `currentPlaylists`，根本到不了 `makeCurrent`，所以把一台已关掉的显示器指向某个播放列表，只有标题变了，屏幕还是黑的 |
...
**这里面有六条是同一个形状，而现在只剩一条了。** K22、K26、K27、K29、K34、K36 是按六份互不相干的报告
立的，其实是一件事：一个只对**某一台显示器**成立的事实，被放进了只装得下一个答案的槽里。这也是为什么
一条一条修总会漏下兄弟，也是为什么其中两条活过了自己的修复而没人发现。已经关掉五条。**K26 是最后一条，
而且它是这个问题的另一半**——那份记录现在按显示器存、跟着 scene 一起清理，缺的是一个把它读出来的展示面。
下一条这个形状的出现时，去找那个槽，不要去找症状。护栏是 `ScopeTests` 和 `SwitchedOffTests`。

**第二个形状，要等第一个清干净才看得见。** K21 和 K23 都是面板与壁纸在「这台显示器正在显示什么」上意见
不一致——一个体现在它画出来的画面上，一个体现在按下一个控件会发生什么上——而 K12 和 K26 是面板与壁纸在
壁纸自己拥有的状态上意见不一致。十条还开着的条目里有四条，是面板对一个 scene 的读法，而不是 scene 本身。
下一轮值得花力气的就是这里，而 `DisplayPanelModel` 是这四条都会碰到的那一个文件。
...
- **K36** 加载错误的记录按显示器为键、跟着 scene 一起清理，所以一台显示器的重载抹不掉另一台的失败。至于有没有人看得见它，那仍然是 K26。
