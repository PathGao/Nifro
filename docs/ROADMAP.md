...
| **W9** | The scaffolding the menu left behind | `SSMenu` is deleted. `CallbackMenuItem.validateCallback` is never assigned so `validateMenuItem` is a constant `true`; `WebsitesScreen` has an `.onChange` whose body is an empty `withAnimation` and an empty `.onAppear` |
