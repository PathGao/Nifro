...
section here was checked against the code it describes. Seven claims turned out to be false and are
...
D8's "still unwatched", and this section's own rule about counts in prose. The pattern in the misses
is one thing: **"nobody has looked" was repeatedly written where "nobody has read the callers" was the
truth**, and reading the callers cost less than the hardware it was waiting for.
...
still and the content moves underneath it**.

**This paragraph was written before L0 shipped, and L0 shipped the other way round.** Moving the
content was tried and is wrong here for a reason that does not apply to a photograph: a web page can
pan and zoom itself, so on floor796 or a map a moving picture has two readings, and the page's own
magnification multiplies with the frame's. A still page and a moving frame has exactly one reading.
The argument survives — there is no gesture conflict, and the table below is what shipped — but the
premise it was reasoned from is the opposite of the code. `Zoom/CropSelectionView.swift` carries the
reasoning that replaced it. **Anything in the L series that inherits "the same two gestures against a
different rectangle" has to inherit the shipped model, not this paragraph.**
...
Escape to put back. ~~Zoom should track the pointer rather than the centre of the screen~~ —
**refuted by the same reversal.** `Geometry.resizedFrame(byGrowing:)` grows around the frame's own
middle on purpose: the page does not move, only the frame does, and anchoring the size change to the
pointer slides the frame out from under it. Pointer-anchored zoom feels right when the picture is
moving and wrong when it is not.
...
|---|---|---|
| **M1** | `localStorage` / IndexedDB | **Yes, but not for the reason this row used to give, and the guarantee is narrower now.** It said `WKWebViewConfiguration()` defaults to the persistent store. It no longer does: `createWebView` assigns `DiskBudget.store(for: website.id)`, so each website gets its own persistent store. floor796 still gets its `last-pos` back. What changed is the scope — **`localStorage` is now partitioned by website entry, not by origin.** Two entries pointing at one site (one per display, say) no longer share a login or a saved position, and deleting an entry and adding the same URL back starts empty, because `removeOrphanedStores` collects the old store at the next launch. That is the price of making a website's disk cost die with the website. The upside on the same trade: "Clear website data" is no longer the *only* way to reclaim it — deleting the website now does, without signing the user out of the ones they kept |
...
| **D2** | A region framed on one display comes out right on the other | **The geometry half has been looked at, and it holds.** `CurrentWebsiteTests` covers framed-on-16:10-shown-on-16:9 directly: `Zoom.region(inPageOfSize:)` derives the region's shape from the page size it is *shown* at, so the region is always the shape of the current display, and `magnification` is recovered from the clamped region. What is still unseen needs two displays and is smaller than it was: whether `pageLayoutSize` picks up the new screen's `pageFrame` after a move, and whether `PageView.layOutContent` re-derives from `bounds` after `installContentView` |
| **D3** | Framing happens on the display the website is on | **The start is fine; the finish has a hole.** `beginCropSelection` matches the scene by website id, and `currentWebsite` *is* `primaryScene.website`, so on the normal path it cannot pick the wrong scene. But `finishCropSelection` finds its way back by `croppingSceneDisplay` and falls back to `primaryScene` — so if that display is unplugged mid-drag, the *main* scene gets its level and interactivity restored while the scene actually being framed stays at `.floating` with `alphaValue = 1`. That is the only path in the app that can pin a window above everything permanently. See K12 |
...
| **D5** | ~~Plugging and unplugging while it runs~~ | **Checked by reading, and the claim is false in three separate ways. Now K13, K14, K15.** "Scenes for displays that went away are torn down" is not what happens: `displaysInUse` reads `website.effectiveDisplay` and never asks whether that display is attached, so `rebuildScenes` keeps the departed display's scene as wanted, and both `DesktopWindow.setFrame` and `WallpaperScene.screen` fall back to `.main` — two full-screen wallpaper windows stacked on the built-in display, each with its own timers and its own menu-bar band. `Display.withFallbackToMain` already exists for exactly this and is called from Settings only. A laptop hits this every time it is undocked, and **it does not take two displays to reproduce**: attach one, point a website at it, unplug |
...
Ordinary use finds these faster than reasoning does — but three of these were found by reading, not
by using, and D2 and D8 turned out not to need a second display at all. **The rule this section was
built on has now failed twice in the same direction: "nobody has looked" kept being used where
"nobody has read the callers" was the truth.** What is genuinely left needs hardware: D1, D4, D6, and
the two remaining halves of D2.
...
The pipeline is right and the numbers are the problem. Every entry was written by an agent
...
| **K6** | The wording is right in places and thin in others | ~~Every setting now says what it does~~ — counted, and that is optimistic: seven `SettingHelp` / `.help(` sites across `Screens/`, against two settings screens of 25 KB. The ones that exist were written one at a time and it shows: they were written one at a time and it shows: some explain the mechanism, some explain the consequence, and a few explain both at different lengths. Worth one pass that reads them as a set rather than as twenty separate answers |
