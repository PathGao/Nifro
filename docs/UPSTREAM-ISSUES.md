...
> **这份文件只记录分析，不记录进度。**
>
> 它写的是每条上游 issue 是什么、归到哪一类、以及为什么这么归。
> **做没做完只在 [ROADMAP.md](ROADMAP.md) 里有答案**，那是唯一的一处。
> 「DO / LATER / REJECT / OBSOLETE」是分类，不是待办状态。
>
> 分诊是在上游 issue 列表的某个时点做的，行号和引用数不会跟着更新。
> 一条 issue 后来被实现、被推翻或者上游自己关掉了，都不反映在这里。
> 想知道现状看 ROADMAP，想知道当初为什么这么判看这里。
