...
**只渲染你看得见的部分。** 壁纸会在你打开的每一个窗口下面继续一帧一帧地画。Nifro 量出它实际还
露出多少，只渲染那一块——小到程序坞后面那一条也算，而那一条恰恰是 macOS 自己永远不会报告为
「已隐藏」的。

**自己判断一个页面需不需要渲染。** 大多数壁纸都是加载、稳定，然后就是一张图。Nifro 先看它一
分钟；什么都不动就改成按计划拍照，而不是让浏览器开一整天。页面要是动起来，它会改回来。

**放大网页的一部分。** 在壁纸上拖一个框，那一部分就铺满屏幕。页面仍然按整屏排版，所以站点不会
重排成你没框过的样子；那一块是重新渲染的而不是拉大的，字仍然清楚。
