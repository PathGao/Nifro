...
Two requests that turned out to be one mechanism (Plash#4). Rotation is the obvious half. The other, asked for in the same thread, is a schedule: a news page in the morning, something calmer at night. Both come down to "which of this display's websites should be showing right now", so they share an answer rather than being two features that fight over the same state.

A website with no hours set is always eligible. If a schedule leaves a display with nothing eligible, the schedule is ignored for that display rather than blanking the wallpaper — an empty desktop is a worse outcome than the wrong page.
*/
extension Website {
	/**
