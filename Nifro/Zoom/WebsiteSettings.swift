...
/**
Where the page has actually ended up, and a way to keep it.

This was an item in the menu bar called "Update Website to Current". It is a permanent edit to a
stored address, made from a menu that gave no sight of the address it was about to write — so the one
thing a person needed in order to decide was the one thing they could not see. It once turned a
website into a GitHub 404.

Here instead, with the address on screen, next to the address it would replace. Absent entirely when
the page is where the website says it should be, which is nearly always.
*/
struct WebsiteNavigatedAddressSetting: View {
	let website: Website

	@State private var navigated: URL?
	...
	var body: some View {
		if let navigated {
			LabeledContent {
				...
				Text("Now showing")
					.explained(String(localized: "Browsing Mode let you move this page somewhere else. Saving that address makes it where the wallpaper starts from now on."))

				Text(navigated.absoluteString)
					.font(.caption)
					.foregroundStyle(.secondary)
					.lineLimit(2)
	...
	private func refresh() {
		navigated = AppState.shared.scenes
			.first { $0.website?.id == website.id }
			.flatMap { $0.webViewController.webView.navigatedURL(for: website) }
	}
}
