...
the settings Oriel should use for it. The app reads this registry to build its built-in gallery, so
adding a good site here is a one-click win for everyone who installs Oriel.

**This is the lowest-effort way to contribute to Oriel.** You do not need Swift, Xcode, or a Mac
...
| `url` | string | yes | What Oriel loads. Include query parameters that are part of the setup (kiosk mode, transparent background, colours). |
...
- **`snapshot`** — the page changes slowly. Oriel renders it, takes a picture, and suspends the web
...
1. **Try it first.** Put the URL in Oriel, live with it for a bit, and work out the settings — the
