...
一批看起来适合当壁纸的页面，以链接的形式留着——因为目前已经确定的也就只有链接。

旁边那些 YAML 文件是另一回事。**一个 YAML 条目是一句断言：有人把这个页面调通了。**它需要多长的
刷新间隔、值得显示的是它的哪一块、哪三行 CSS 能藏掉它的导航、它应该持续渲染还是按计划拍照。
那才是工作量，也是不值得做第二遍的部分。一条链接承载不了它，CI 也校验不了它。
...
这里          一条网址和一句话
  ↓           有人把它的设置调出来，并验证成立
<name>.yml    经 schema 校验的设置，出现在 app 的图库里
  ↓           被挑进「值得随包发布」的那几条
featured      首次启动时装好，之后就是一个用户可编辑、可删除的普通网站
```

在这里加一条链接是受欢迎的，而且很便宜。让一条毕业，价值更高。
...
筛选基准来自这份清单最初的那几条： `Windy`、`World Monitor`、`Floor796`、`A Genealogy of Technology and Power since 1500`、`anime.js`：要么让真实系统显形，要么提供可漫游的网络空间，要么把运动做得克制且有生命感。
